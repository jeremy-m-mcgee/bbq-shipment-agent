...
- Two controls, no overlap: the **judge config** decides whether scoring happens (enable/disable/target it in LD), the **flag** decides whether a low score withholds the reply. "Shadow" is not a mode — it is judge enabled with the flag off, and it is where a rollout starts.
- Only one path suppresses: judge ran, returned a score, score below `THRESHOLD`, flag on. Unavailable judge, sampled-out, errored, missing score — all pass and are recorded. A guard that blanks a legitimate narration is worse than the answer it prevents.
- The threshold is a Python constant. A threshold is control flow; the mode, model and rubric are LD's.
- Enforcing turns off streaming. A reply that must be judged before the operator sees it cannot already be on their screen, so the pane drops `data-stream` and posts to the sync route.
