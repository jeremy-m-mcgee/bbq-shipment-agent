...
- **The judge is looked up per scored turn, never held.** `create_judge` evaluates the config once and freezes `enabled`, the model and the rubric into the `Judge`, so an instance built at review start is deaf to LaunchDarkly for the whole review — and a parked review can sit for a long time. `ScopeGuard` therefore takes a *factory* and calls it at the top of each `check`. This is the one deliberate exception to "identity comes from the config captured at A1": that rule is about agent configs, and the judge is not one. The consequence to keep: two turns of one review may carry two different judge instruction hashes, which is the point rather than a defect.
- `ScopeGuard.enforcing` is the weaker claim — *there is a client to ask* — not *this turn will be scored*, which only LaunchDarkly knows and is asked per turn. So a live client suppresses streaming even when the judge config turns out to be disabled. Trading that for an accurate flag means probing LD on every pane render.
