...
- `tests/fixtures/shippo-*.json` — the same format as that cache, because a recording *is* one. `tools/record_shippo.py` refills them by running the CLI with `--cache` pointed at a scratch copy; it never invents an answer and never calls a model.
...
- `RunDepth.EXTRACT` stops after B1 — inside `build_roster`, the one place the pipeline naturally ends. It never reaches C2, which is what makes it the *cheap* offline screenshot run: one vision call per image and no quote at all. Full depth is offline too now that `shippo-quotes-screenshots.json` holds the lanes, but it replays a few hundred recorded quotes to say the same thing about a prompt.
