...
It used to stop where `run plan` stops: no approval button, no edits. It does not any more — D2 arrived in the browser as this section anticipated. A run that produces a covering manifest parks in an `awaiting_review` state and the `/runs/{id}/review/*` routes drive the conversation, the edits and the terminal states; the result page keeps its row model the same as `ReviewSession.manifest`, so the review is a pane beside the manifest rather than a second manifest, re-rendered in place as edits re-solve. What did *not* move is the boundary: section 4's edit-handling table is still Python's, `ReviewSession` still owns every decision, and the routes only carry the operator's move to it. The narrator, when a model is available, streams its reply token by token over SSE and falls back to a button-driven review offline — the same fallback the CLI takes when `review-narrator` is unavailable. Approval writes the ledger from the browser, which is the same trust as the CLI: loopback only, no auth, one operator, one run at a time (a parked review holds the run slot until it reaches a terminal state, so a second run cannot append to the same ledger underneath it).
