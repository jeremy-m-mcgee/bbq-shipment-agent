...
**B3's offline replay now reproduces its live behaviour — resolved by recording it.** `tests/fixtures/b3-repairs.json` records a real repair conversation, and the tool results are recomputed on each run rather than recorded, which is the right design: the validator and the image cropper are genuinely exercised. The half that was missing was the other side of that. `shippo-addresses.json` held no recording for the addresses the *agent proposed*, so `RecordedAddressValidator` raised on them, `_adjudicate` caught the raise and called the proposal rejected, and a replay that should have repaired 6 and escalated 3 repaired roughly 1 and escalated 8.

Every proposal the recorded conversation makes — the ones it passes to `validate_address` and the ones `_adjudicate` re-validates at the end — is now in that fixture, put there by `tools/record_shippo.py` against the live validator. `tests/test_screenshot_replay.py` asserts the counts rather than the shape.

Two things from the episode are worth keeping. `_adjudicate` cannot tell "the validator refused this address" from "nothing ever recorded this address", because both arrive as an exception it is right to catch and right not to let kill the loop — so a missing fixture presented itself as a plausible repair failure. And the tests passed throughout, because they asserted that everyone was accounted for and that *some* repairs happened. Shape assertions are how a fixture regression stays invisible; the new test therefore pins the count and, separately, that nothing was rejected for want of a recording.

One thing the recording does not fix, and should not be read as fixing: `b3-repairs.json` predates `08-imessage-cousins.png`, so the recipient that screenshot added gets no verdict from the conversation and stays escalated with "the repair loop returned no verdict for this record". That is the replay being honest about a conversation it does not have. Re-recording it costs a live vision conversation rather than a Shippo call, which is why it is a separate job from this one.
...
What that did not buy, for a while, was a fully offline screenshot run *at full depth*. `shippo-quotes-sf-dc.json` held one lane while the fixture screenshots hold twenty-odd destinations, so a replayed screenshot **plan** reached C2 and stopped with `RecordedQuoter` refusing to invent a rate — correct behaviour, and the reason the offline demo path was the roster file rather than the pictures.

**Those lanes are recorded now**, in `shippo-quotes-screenshots.json`, and it was the same job as recording B3's proposals above because both are answers only the live API can give. `tools/record_shippo.py` is that job written down. It is not a third front-end: it runs the CLI, the way `drive` posts at the UI, with `--cache` pointed at a scratch directory seeded from the fixtures — the quoter's cache format *is* the fixture format, which is what makes a recording a copy rather than a conversion, and the seeding is why re-recording after a new screenshot pays for that screenshot's lanes and nothing else.

Three passes, because no single run reaches everything. The set B2 hands to B3 is a function of `validation-mode`, so a **strict** pass is what makes the repair loop propose at all; a **standard** pass is what quotes the lane each corrected address actually ships to; and a third pass feeds B3's repaired addresses back in as an ordinary roster, because those are different doorsteps — one moves a Chicago recipient from 60631 to 60613 — and nothing quotes them until `planner-mode` is `on`, which 6.9 will not allow until five shadow runs exist.

What has not changed is that the quoter still refuses to invent a rate. The origin is half of a quote key, so a replay from a different origin raises rather than answering, and `roster-screenshots.yaml` exists to state the one every recorded rate was measured from.

**Depth resolved the demo half first, and it needed no new recording.** The limit is a fact about the *quoter*, so a run that never reaches C2 never meets it: `RunOptions.depth` stops a run after B1, and a replayed screenshot extraction is completely offline against the fixtures that already exist. That path was already in the CLI as `run extract` — B1 runs inside `build_roster` and nothing after it does, which is a real seam rather than a stage counter — and what changed is that the seam moved into `wiring.extract_with`, where the browser and the driver reach it too.
...
Depth is therefore the *cheap* offline screenshot run rather than the only one. The full-depth path is offline too, and the difference between them is what it costs to learn something: an extract-only run makes one vision call per image and no quote at all, while a full plan replays a few hundred recorded quotes to reach a manifest that says nothing about a prompt.

That second gap is closed rather than unchanged, and closing it mattered for the reason it was visible: a demo run through the UI showed the wrong escalation list to whoever was watching. It now shows the one the live run produced.
