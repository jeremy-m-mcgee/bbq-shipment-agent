...
5. **Extraction.** B1, screenshots to records. *Blocked: needs screenshots.* The brief for producing them is `docs/screenshot-fixtures.md`, which is a staging document and gets deleted once `tests/fixtures/screenshots/` exists.
...
7. **Repair loop.** B3, the first tool-using agent, and the one with the clearest payoff. *Blocked on step 5* — the loop re-reads the source image region, so `read_image_region` cannot exist until B1 produces provenance pointers. Its AI Config is live and `validate_address` is already registered.
