...
from bbq_shipment_agent.capabilities import GuardMode
...
def narrator(session, tmp_path, model, judge, mode=GuardMode.ON):
    ...
    return Narrator(
        ...
        ledger_root=tmp_path / "ledger",
        model=model,
        guard=ScopeGuard(run, judge, ledger_root=tmp_path / "ledger", mode=mode),
    )
...
class TestShadow:
    ...
    def test_it_scores_and_records_but_shows_the_reply(self, session, tmp_path):
        ...
        assert turn.refused is False
        scored = invocations(tmp_path, JUDGE_KEY)
        assert [r.outcome for r in scored] == ["out_of_scope"]
        assert scored[0].judge_score == 0.0

    def test_the_conversation_is_left_alone(self, session, tmp_path):
        judge = ScriptedJudge(_result(0.0))
        voice = narrator(
            session, tmp_path, ScriptedModel(Completion(text="leetcode")), judge,
            mode=GuardMode.OFF,
...
class TestTheCapabilityStaysOutOfTheFingerprint:
    def test_guard_is_not_in_the_snapshot(self, session, tmp_path):
        # The load-bearing decision. `resolved_capabilities` needs every
        # registered capability before it produces a fingerprint, and `guard`
        # is read at D2 -- after `_record_planning` has already folded the set.
        # Registering it would null every run row's `cap_snapshot`.
        run, _review = session
        run.planner()
        run.validation()
        run.verification()
        ...
        assert sorted(run.cap_snapshot()["capabilities"]) == [
            "planner",
            "validation",
            "verification",
        ]
